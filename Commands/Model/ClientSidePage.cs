...
namespace SharePointPnP.PowerShell.Core.Model
{
    ...
    public class ClientSidePage
    {
        ...
        private SPOnlineContext context;
        ...
        /// <param name="clientSidePageLayoutType"><see cref="ClientSidePageLayoutType"/> type of the page to create. Defaults to Article type</param>
        public ClientSidePage(SPOnlineContext cc, ClientSidePageLayoutType clientSidePageLayoutType = ClientSidePageLayoutType.Article) : this(clientSidePageLayoutType)
        {
            if (cc == null)
            {
        ...
        public string PagesLibrary
        {
            ...
            set
            {
                ...
                if (this.Context != null)
                {
                    try
                    {
                        var list = new RestRequest(Context, $"Web/Lists/GetByUrl('{value}')").Get<List>();
                    }
                    catch (Exception)
                    {
        ...
        /// </summary>
        public ListItemAllFields PageListItem
        {
            get
            {
        ...
        //internal static string ClientSideWebPartEnumToName(DefaultClientSideWebParts image)
        //{
        ...
        //        {
        //            this.pageListItem = new RestRequest(Context, this.PageListItem).Select("CommentsEnabled").Get<Client>();
        ...
        public void Delete()
        {
            ...
            new RestRequest(Context, this.pageListItem).Delete();
        }
        ...
        public static ClientSidePage Load(SPOnlineContext cc, string pageName)
        {
            if (cc == null)
            {
                throw new ArgumentNullException("Passed ClientContext object cannot be null");
            ...
            var pagesLibrary = new RestRequest(cc, $"Web/GetList('{UrlUtility.Combine(cc.Web.ServerRelativeUrl, page.PagesLibrary)}')").Expand("RootFolder").Get<List>();
            ...
            var file = new RestRequest(cc, $"Web/GetFileByServerRelativeUrl('{page.sitePagesServerRelativeUrl}/{page.pageName}')").Expand("Exists", "ListItemAllFields").Get<File>();
            ...
            var item = new RestRequest(cc, file.ListItemAllFieldsOdataNavigationLinkUrl).Expand(ClientSidePage.ClientSideApplicationId, ClientSidePage.PageLayoutType, ClientSidePage.CanvasField).Get<ListItemAllFields>();

            // Check if this is a client side page
            if (item.FieldValues.ContainsKey(ClientSidePage.ClientSideApplicationId) && item.FieldValues[ClientSideApplicationId] != null && item.FieldValues[ClientSideApplicationId].ToString().Equals(ClientSidePage.SitePagesFeatureId, StringComparison.InvariantCultureIgnoreCase))
            {
                page.pageListItem = item;
                page.PageTitle = Convert.ToString(item.FieldValues[ClientSidePage.Title]);

                // set layout type
                if (item.FieldValues.ContainsKey(ClientSidePage.PageLayoutType) && item.FieldValues[ClientSidePage.PageLayoutType] != null && !string.IsNullOrEmpty(item.FieldValues[ClientSidePage.PageLayoutType].ToString()))
                {
                    page.LayoutType = (ClientSidePageLayoutType)Enum.Parse(typeof(ClientSidePageLayoutType), item.FieldValues[ClientSidePage.PageLayoutType].ToString());
                }
                else
                {
                ...
                if (item.FieldValues.ContainsKey(ClientSidePage.CanvasField) && !(item.FieldValues[ClientSidePage.CanvasField] == null || string.IsNullOrEmpty(item.FieldValues[ClientSidePage.CanvasField].ToString())))
                {
                    var html = item.FieldValues[ClientSidePage.CanvasField].ToString();
        ...
        public System.Collections.Generic.IEnumerable<ClientSideComponent> AvailableClientSideComponents(string name)
        {
            ...
            var clientSideComponents = new RestRequest(Context, "Web/GetClientSideWebParts").Get<ResponseCollection<ClientSideComponent>>().Items;
        ...
        private void LoadPageFile(string pageName, out string serverRelativePageName, out File pageFile)
        {
            ...
            {
                this.spPagesLibrary = new RestRequest(Context, $"Web/Lists/GetByUrl('{this.PagesLibrary}')").Expand("RootFolder").Get<List>();
            ...
            // ensure page exists
            pageFile = new RestRequest(Context, $"Web/GetFileByServerRelativeUrl('{serverRelativePageName}')").Expand("ListItemAllFields", "Exists").Get<File>();
        }
        ...
        //private async Task<string> GetClientSideWebPartsAsync(string accessToken, SPOnlineContext context)
        //{
