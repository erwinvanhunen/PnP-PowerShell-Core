...
namespace SharePointPnP.PowerShell.Core.Web
{
    ...
    public class GetSubWeb : PnPCmdlet
    {
        ...
        {
            var webs = new RestRequest(CurrentContext, "Web/Webs").Expand("AllowAutomaticASPXPageIndexing", "AllowCreateDeclarativeWorkflowForCurrentUser", "RequestAccessEmail", "ServerRelativeUrl").Get<ResponseCollection<Model.Web>>().Items;
            if (!Recurse)
            {
        ...
        private System.Collections.Generic.List<Model.Web> GetSubWebsInternal(Model.Web subweb)
        {
            ...
            var webs = new RestRequest(CurrentContext, $"{subweb.Url}/_api/Web/Webs").Get<ResponseCollection<Model.Web>>().Items;
            if (webs.Any())
            {
                subwebs.AddRange(webs);
