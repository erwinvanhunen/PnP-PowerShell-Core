        ...
        protected override void ExecuteCmdlet()
        {
            if (ParameterSetName == "List")
            {
                ...
                if (list != null)
                {
                    ...
                    {
                        WriteObject(new RestRequest(CurrentContext, $"Web/Lists/GetById(guid'{list.Id}')/EventReceivers").Get<ResponseCollection<EventReceiver>>().Items, true);
                    }
                    else
                    {
            ...
            else
            {
                if (!MyInvocation.BoundParameters.ContainsKey("Identity"))
                {
                    WriteObject(new RestRequest(CurrentContext,$"Web/EventReceivers").Get<ResponseCollection<EventReceiver>>().Items, true);

                }
                else
                {
                    WriteObject(Identity.GetEventReceiverOnWeb(CurrentContext));
                }
            }
        }
    }
