...
namespace SharePointPnP.PowerShell.Core.ModuleFilesGenerator
{
    internal class CmdletsAnalyzer
    {
        ...
        internal List<Model.CmdletInfo> Analyze()
        {

            return GetCmdlets();

        }
        private List<Model.CmdletInfo> GetCmdlets()
        {
            List<Model.CmdletInfo> cmdlets = new List<Model.CmdletInfo>();

            var types = _assembly.GetTypes().Where(t => t.BaseType != null && (t.BaseType.Name.StartsWith("PnP") || t.BaseType.Name == "PSCmdlet")).OrderBy(t => t.Name).ToArray();
...
#pragma warning restore CS0612 // Type or member is obsolete
                    ...
                    {
                        var a = helpAttribute;
                        if(cmdletInfo.Verb == null)
                        {
                            cmdletInfo.Verb = a.Verb;
                        }
                        if(cmdletInfo.Noun == null)
                        {
                            cmdletInfo.Noun = a.Noun;
                        }
